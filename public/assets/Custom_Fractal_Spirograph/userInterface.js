...
function uiSetup() {
  ...
  tikv = createSlider(-4, 3, -2, 0);
  tikv.changed(() => {
    reset();
  });
  stepv = createSlider(1, 2000, 100);
  stepv.changed(() => {
    reset();
  });
  ...
  flp2 = createButton('Alternate Inside/Outside: OFF');
  flp2.mousePressed(() => {
    flip2Overide *= -1;
    if (flip2Overide == -1) {
      flp2.html('Alternate Inside/Outside: ON');
    } else {
      flp2.html('Alternate Inside/Outside: OFF');
    }
    reset();
  })
  ins = createButton('Inside/Outside: INSIDE');
  ins.mousePressed(() => {
    insideC = !insideC;
    if (!insideC) {
  ...
  dark = createSlider(0, 100, 90, 0);
  dark.changed(() => stroke(0, 0, 0, dark.value()));
