...
function setup() {
    ...
    keyRender = new keyRenderer(emulatedKeys);
...
function draw() {
    ...
    keyRender.setPosition(width - 340, 20);
    keyRender.render();
...
function drawAnalyser()
{
    ...
    let margin = iterSlider.y;
    translate(-width * 0.5, -height * 0.5 + margin);
    ...
    beginShape();    
   	for (i = 1; i < spectrum.length; i++) {
        var sliceWidth = ((width * 2) / bufferLength) * 10; // width * 1.0 / spectrum.length;
        vertex(x, map(spectrum[i], 0, 255, height / 8,0 ) );
    	x += sliceWidth * (10/i);
   	}
   	endShape();
}
...
function updateTime() {
    if (playBtn.isPlaying) time += getPhaseInc(frequency * 10);
...
function getNoteForFrequency(f)
{
...
function keyPressed() {
    ...
    if (key == "z") {
        octave -= 12;
        keyRender.setOctave(-12);
        
        let note = getNoteForFrequency(audioFrequency) + octave;
        noteOn(note);
    }
    ...
    if (emulatedKeys.hasOwnProperty(key)) {
        ...
        console.log(getNoteForFrequency(f));
    }
    
    keyRender.setCurrentKey(key);
}
...
class keyRenderer {
    ...
    setCurrentKey(key)
    {
        ...
        if (this.keyset.hasOwnProperty(key)){
        	this.lastNote = this.keyset[key];
        }
    }
    ...
    drawNote()
    {
        ...
        let note = this.lastNote;
        note += this.octave;
        text(this.getNoteName(note), x + (size/2), y +(size/2)); 
    }
    ...
    render() 
    {
        ...
        for (var value in this.keyset) 
        {
            fill(220);
            if (value == this.currentKeyPressed){
                fill(12, 220, 12);
            }
            
            rect(x_in, y_in, size, size, size / 4);
            
            
            textAlign(CENTER, CENTER);
            textSize(size / 2);
            fill(12);
            text(value, x_in + (size/2), y_in + (size/2));
        ...
        y_in += size + spacing;
        x_in = this.x + size/2; //offset
        for (i = 0; i < this.octaveKeys.length; i++)
        {
            fill(220);
            if (this.octaveKeys[i] == this.currentKeyPressed){
                fill(12, 220, 12);
            }
            ...
            textSize(size / 2);
            fill(12);
            text(this.octaveKeys[i], x_in + (size/2), y_in + (size/2) );
...
function drawText() {
    ...
    text(
        "frequency",
        freqSlider.x + freqSlider.width + spacing,
        freqSlider.y + (fontSize/3)
    );
    text(
        "harmonics",
        iterSlider.x + iterSlider.width + spacing,
        iterSlider.y + (fontSize/3)
    );
    text(
        "volume",
        volumeSlider.x + volumeSlider.width + spacing,
        volumeSlider.y  + (fontSize/3)
    );
    text(
...
function drawWave() {
    ...
    if (mousedown) {
        ...
        let a = time;
        let delta_a = abs(old_angle - a) * 10000;
        old_angle = a;
...
function drawContourShape() {
    ...
    endShape();
    if (contourShape.length > 60 / frequency) contourShape.shift();
}
...
function drawWaveshape() {
    ...
    while (waveShape.length > width - 300) waveShape.pop();
}
...
class triangleWaveObject extends waveObject {
    getTransform(n) {
        return 8 / (PI * PI) * pow(-1, (n - 1) / 2) / (n * n);
    }
...
class smoothParameter {
    ...
    getSmoothed() {
        if (this.countdown <= 0) return this.targetVal;
    ...
    setValue(value) {
        if (this.targetVal != value) {
            ...
            if (this.countdown <= 0) this.curVal = this.targetVal;
            else this.step = (this.targetVal - this.curVal) / this.countdown;
        }
    }
...
class playButton {
    ...
    clicked(mX, mY) {
        ...
        if (distance <= rad) {
            ...
            else if (this.isPlaying == false) this.isPlaying = true;
        }
    }
